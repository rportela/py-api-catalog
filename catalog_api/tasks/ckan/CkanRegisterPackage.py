...
from catalog_api.tasks.ckan.CkanRegisterResource import CkanRegisterResource


class CkanRegisterDataset:
    def __init__(self):
        self._register_license = CkanRegisterLicense()
        self._register_resource = CkanRegisterResource()
    ...
    def execute(self, org: CatalogGroup, pkg: CkanPackage) -> CatalogDataset:
        ...
        datasets = self._service.query_datasets_by_name_and_org(
            catalog_name, str(org.id)
        )
        dataset = datasets[0] if len(datasets) > 0 else None
        if not dataset:
            license = self._register_license.register_license(
                license_id=pkg.license_id, title=pkg.license_title, url=pkg.license_url
            )
